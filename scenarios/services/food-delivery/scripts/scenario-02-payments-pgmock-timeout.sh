...
measure_baseline() {
    ...
    restaurant_time=$(curl -s -o /dev/null -w "%{time_total}" \
        "$API_BASE/api/restaurants" 2>/dev/null || echo "N/A")
    log_info "베이스라인 상품조회 응답시간: ${restaurant_time}s" >&2
...
send_concurrent_orders() {
    ...
    for i in $(seq 1 "$CONCURRENT_ORDERS"); do
        curl -s -o /tmp/scenario-02-order-${round}-${i}.log \
            ...
            -X POST "$API_BASE/api/orders" \
            -H "Content-Type: application/json" \
            -d "{\"customerName\":\"timeout-${round}-${i}\",\"customerEmail\":\"timeout${i}@test.com\",\"items\":[{\"restaurantId\":$((i % 16 + 1)),\"quantity\":1}]}" \
    ...
    local restaurant_code restaurant_time
    restaurant_code=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 "$API_BASE/api/restaurants" 2>/dev/null || echo "000")
    restaurant_time=$(curl -s -o /dev/null -w "%{time_total}" --max-time 10 "$API_BASE/api/restaurants" 2>/dev/null || echo "timeout")
    log_info "GET /api/restaurants: HTTP $restaurant_code in ${restaurant_time}s (이 경로는 정상이어야 함)"
...
check_service_status() {
    ...
    for svc in "order:8080" "restaurant:8081" "orders:8082" "payment:8083"; do
        local name=${svc%%:*}
        local port=${svc##*:}
        local pod
...
cleanup() {
    ...
    order_code=$(curl -s -o /dev/null -w "%{http_code}" --max-time 30 \
        -X POST "$API_BASE/api/orders" \
        -H "Content-Type: application/json" \
        -d '{"customerName":"recovery-test","customerEmail":"recover@test.com","items":[{"restaurantId":5,"quantity":1}]}' 2>/dev/null || echo "000")
