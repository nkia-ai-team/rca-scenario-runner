...
check_prerequisites() {
    ...
    kubectl -n "$NAMESPACE" exec "$PG_POD" -- \
        ...
        -c "UPDATE orders_schema.orders SET stock = 9999;" 2>/dev/null
...
send_orders() {
    ...
    # 다양한 상품에 대해 주문 (hot restaurant + 분산)
    for i in $(seq 1 "$concurrent"); do
        local restaurant_id
        if (( i % 2 == 0 )); then
            restaurant_id=1  # 인기 상품 (50%가 동일 상품 집중 → row lock contention)
        else
            restaurant_id=$(( (i % 16) + 1 ))  # 나머지 분산
        fi
        ...
        curl -s -o /dev/null \
            ...
            -d "{\"customerName\":\"flood-p${phase}-${i}\",\"customerEmail\":\"flood${i}@test.com\",\"items\":[{\"restaurantId\":${restaurant_id},\"quantity\":1}]}" \
...
take_snapshot() {
    ...
    kubectl --kubeconfig=/home/nkia/.kube/config -n "$NAMESPACE" top pod --no-headers 2>/dev/null | \
        grep -E "order|restaurant|orders|payment" | \
...
cleanup() {
    ...
    kubectl -n "$NAMESPACE" exec "$PG_POD" -- \
        psql -U plopvape -d plopvape \
        -c "UPDATE orders_schema.orders SET stock = 50 WHERE restaurant_id <= 4;
            UPDATE orders_schema.orders SET stock = 200 WHERE restaurant_id BETWEEN 5 AND 8;
            UPDATE orders_schema.orders SET stock = 300 WHERE restaurant_id BETWEEN 9 AND 12;
            UPDATE orders_schema.orders SET stock = 100 WHERE restaurant_id BETWEEN 13 AND 16;" 2>/dev/null || true
    ...
    local services=(testbed-order testbed-restaurant testbed-orders testbed-payment testbed-notification)
