...
# =============================================================================
# Root Cause: orders 테이블에 장시간 SELECT FOR UPDATE lock을 점유한 상태에서
#             동시 주문을 발생시켜 row lock contention을 재현한다.
#
# 전파 경로: orders lock wait → restaurant timeout → order 5xx → Nginx 502
#
# 사용법:
#   ./scenario-01-orders-lock.sh          # 시나리오 실행
#   ./scenario-01-orders-lock.sh cleanup  # 원상복구
#
...
LOCK_RESTAURANT_ID=1          # lock을 걸 상품 ID
...
check_prerequisites() {
    ...
    stock=$(kubectl -n "$NAMESPACE" exec "$PG_POD" -- \
        ...
        -c "SELECT stock FROM orders_schema.orders WHERE restaurant_id = $LOCK_RESTAURANT_ID;" 2>/dev/null)
    log_info "상품 $LOCK_RESTAURANT_ID 현재 재고: $stock"

    if [[ "$stock" -lt 5 ]]; then
        log_warn "재고가 부족합니다. 재고를 100으로 리셋합니다."
        kubectl -n "$NAMESPACE" exec "$PG_POD" -- \
            psql -U plopvape -d plopvape \
            -c "UPDATE orders_schema.orders SET stock = 100 WHERE restaurant_id = $LOCK_RESTAURANT_ID;" 2>/dev/null
    fi
...
measure_baseline() {
    ...
    total_time=$(curl -s -o /dev/null -w "%{time_total}" \
        ...
        -d "{\"customerName\":\"baseline\",\"customerEmail\":\"baseline@test.com\",\"items\":[{\"restaurantId\":$LOCK_RESTAURANT_ID,\"quantity\":1}]}" 2>/dev/null || echo "N/A")
...
start_lock_transaction() {
    log_info "=== LOCK 트랜잭션 시작 (restaurant_id=$LOCK_RESTAURANT_ID, ${LOCK_DURATION}초) ==="

    # 백그라운드에서 장시간 lock을 잡는 트랜잭션 실행
    kubectl -n "$NAMESPACE" exec "$PG_POD" -- \
        psql -U plopvape -d plopvape \
        -c "BEGIN;
            SELECT id, restaurant_id, stock FROM orders_schema.orders
            WHERE restaurant_id = $LOCK_RESTAURANT_ID FOR UPDATE;
    ...
    lock_count=$(kubectl -n "$NAMESPACE" exec "$PG_POD" -- \
        psql -U plopvape -d plopvape -t -A \
        -c "SELECT count(*) FROM pg_locks WHERE relation = 'orders_schema.orders'::regclass;" 2>/dev/null || echo "0")

    if [[ "$lock_count" -gt 0 ]]; then
        log_ok "Row lock 확인됨 (locks: $lock_count)"
        kubectl -n "$NAMESPACE" exec "$PG_POD" -- \
            psql -U plopvape -d plopvape \
            -c "SELECT locktype, mode, granted, pid FROM pg_locks WHERE relation = 'orders_schema.orders'::regclass;" 2>/dev/null || true
    else
...
send_concurrent_orders() {
    ...
    for i in $(seq 1 "$CONCURRENT_ORDERS"); do
        curl -s -o /tmp/scenario-01-order-${round}-${i}.log \
            ...
            -X POST "$API_BASE/api/orders" \
            -H "Content-Type: application/json" \
            -d "{\"customerName\":\"locktest-${round}-${i}\",\"customerEmail\":\"lock${i}@test.com\",\"items\":[{\"restaurantId\":$LOCK_RESTAURANT_ID,\"quantity\":1}]}" \
...
cleanup() {
    ...
    kubectl -n "$NAMESPACE" exec "$PG_POD" -- \
        psql -U plopvape -d plopvape \
        -c "UPDATE orders_schema.orders SET stock = 50 WHERE restaurant_id = $LOCK_RESTAURANT_ID;" 2>/dev/null
    log_ok "상품 $LOCK_RESTAURANT_ID 재고를 50으로 복원"
