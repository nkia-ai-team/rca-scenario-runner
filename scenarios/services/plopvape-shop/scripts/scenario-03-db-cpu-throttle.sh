...
measure_baseline() {
    ...
    db_time=$(kubectl -n "$NAMESPACE" exec "$PG_POD" -- \
        ...
        -c "\\timing on" \
        -c "SELECT count(*) FROM product_schema.products p JOIN inventory_schema.inventory i ON p.id = i.product_id;" 2>&1 | grep "Time:" | awk '{print $2}' || echo "N/A")
    log_info "베이스라인 DB 쿼리 시간: ${db_time:-N/A} ms" >&2

    echo "$product_time|$order_time"
}
        ...
        for i in $(seq 1 "$CONCURRENT_REQUESTS"); do
            if (( i % 3 == 0 )); then
                ...
                curl -s -o /dev/null -w "POST-order-${round}-${i}: HTTP %{http_code} in %{time_total}s\n" \
                    ...
                    -X POST "$API_BASE/api/orders" \
                    -H "Content-Type: application/json" \
                    -d "{\"customerName\":\"throttle-${round}-${i}\",\"customerEmail\":\"t${i}@test.com\",\"items\":[{\"productId\":$((i % 16 + 1)),\"quantity\":1}]}" \
                    >> /tmp/scenario-03-results.log 2>&1 &
            else
...
check_db_cpu_status() {
    ...
    kubectl -n "$NAMESPACE" exec "$PG_POD" -- \
        psql -U plopvape -d plopvape \
        -c "\\timing on" \
        -c "SELECT count(*) FROM product_schema.products p JOIN inventory_schema.inventory i ON p.id = i.product_id;" 2>/dev/null
}
